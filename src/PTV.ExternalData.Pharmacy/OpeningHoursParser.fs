...
module OpeningHoursParser =
  ...
  let parseAdditionalInformationHour (input: string option) =
    ServiceLocationJson.ServiceHour
      (
        additionalInformation = CommonParser.parseAdditionalInfo input additionalInfoLength,
        isAlwaysOpen = false,
        isClosed = false,
        isReservation = false,
        validForNow = false,
    ...
    seq {
      ...
      then
        ...
        yield ServiceLocationJson.ServiceHour
                (
                  additionalInformation = CommonParser.parseAdditionalInfo header additionalInfoLength,
                  isAlwaysOpen = false,
                  isClosed = false,
                  isReservation = false,
  ...
  let parseSpecialHours (infoTitle: string) (input: PharmaciesXml.Aukioloaika) =
    seq {
      ...
      if List.isEmpty daysOfTheWeek
      then yield ServiceLocationJson.ServiceHour
                  (
                    additionalInformation = CommonParser.parseAdditionalInfo header additionalInfoLength,
                    isAlwaysOpen = false,
                    isClosed = false,
                    ...
                    validTo = (Option.bind (fun value -> Some(DateTimeOffset(value))) input.Loppupvm.Value),
                    serviceHourType = Defaults.serviceHoursType,
                    openingHour = (List.toArray daysOfTheWeek)
                  )
      if input.Lisatietoja |> Option.defaultValue "" |> String.IsNullOrWhiteSpace |> not
      then yield parseAdditionalInformationHour input.Lisatietoja      
    }
  ...
  let parseHoliday (input: PharmaciesXml.Poikkeusaika) =
    ...
    ServiceLocationJson.ServiceHour
      (
        additionalInformation = CommonParser.parseAdditionalInfo holidayName additionalInfoLength,
        isAlwaysOpen = false,
  ...
  let parseExceptionalHours (daysOfTheWeek: PharmaciesXml.Aukioloaika list) (holidays: PharmaciesXml.Poikkeusaika list) =
    let resultDays = daysOfTheWeek |> List.collect (fun item -> parseSpecialHours "Poikkeusjakso" item |> List.ofSeq)
  ...
  let parseServiceHours (pharmacy: PharmaciesXml.Apteekki) =
    ...
    let overMidnightResult = overMidnightHours
                              |> List.collect (fun item -> parseSpecialHours "Päivystys" item |> List.ofSeq) 
