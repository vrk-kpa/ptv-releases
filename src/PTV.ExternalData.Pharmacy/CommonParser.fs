...
module CommonParser =      
  ...
  let formatEChannelWebPage (url: string) =
    ...
    parseName formattedWithDot 500
        ...
        [|
          ServiceLocationJson.PhoneNumber
            (
              ...
              chargeDescription = fillShortString chargeInfoValue minLengthOfChargeInfoValue chargeType,
  ...
  let parseLanguages (languages: string option) (languageCodes: Dictionary<string, string>) =
    (Option.defaultValue "" languages).Split ";"
    |> Seq.map (fun lang ->
