...
module Importer =
  let private putConnectionRequest (channelIds: Guid list) =
    let channelRelations = 
      channelIds
      |> List.map (fun id ->
                      ConnectionJson.ChannelRelation
                        (
    ...
    ConnectionJson.Service
      (
        ...
        channelRelations = (channelRelations |> Array.ofList)
      )
  
  let private putConnectionRequests (channelIds: Guid[]) =
    channelIds
    |> Utils.batch 25
    |> List.map putConnectionRequest
...
//        |> Async.Ignore
      ...
      let connectionRequestList = putConnectionRequests existingIds
      let connectionUrl = secret.PutConnectionUrl + secret.ServiceId
      do! connectionRequestList
        |> List.mapi (fun index request -> OpenApiClient.putConnection token connectionUrl (request.ToString()) builder index)
        |> Async.Sequential
        |> Async.Ignore
