...
export const useArchiveService = (id: string): UseMutationResult<ServiceApiModel, HttpError, OptionalLanguage, unknown> => {
  const mutation = useMutation<ServiceApiModel, HttpError, OptionalLanguage, unknown>((language: OptionalLanguage) => {
    const url = !language ? `next/service/archive/${id}` : `next/service/archive/${id}?language=${language}`;
