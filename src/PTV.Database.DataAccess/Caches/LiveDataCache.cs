        ...
        protected override LiveCacheResult<Guid, OrganizationTreeItem> LoadData()
        {
            ...
            resolveManager.RunInScope((rm) =>
            {
                ...
                var allOrgs = contextManager.ExecuteReader(unitOfWork =>
                {
                    ...
                    var publishedOrgs = orgRep.All().Where(i => i.PublishingStatusId == psPublished).GroupBy(x => x.UnificRootId).Select(x => x.OrderByDescending(i => i.Modified).FirstOrDefault());
                    var lastestChange = publishedOrgs.OrderByDescending(i => i.Modified).Select(i => i.Modified).Take(1).FirstOrDefault();
