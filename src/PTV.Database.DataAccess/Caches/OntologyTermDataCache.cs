...
namespace PTV.Database.DataAccess.Caches
{
    ...
    internal class OntologyTermDataCache : LiveDataCache<Guid, OntologyTerm>, IOntologyTermDataCacheInternal, IOntologyTermDataCache
    {
        ...
        public HashSet<string> GetOntologyUrisForExactMatches(string exactMatchesUri)
        {
            GetData();
            return exactMatches.TryGetOrDefault(exactMatchesUri, new List<OntologyTerm>())
                .Where(i => i != null)
                .Select(x => x.Uri)
                .ToHashSet();
        }
        ...
        public List<OntologyTerm> SearchOntologyTermByName(string name, string orderLanguageCode, List<Guid> languagesIds = null)
        {
            GetData();

            return dataByName
                .Where(x => x.Key.Name.Equals(name, StringComparison.InvariantCultureIgnoreCase) &&
                            (languagesIds.IsNullOrEmpty() || languagesIds.Contains(x.Key.LanguageId)))
                .OrderBy(x => x.Key.Name, StringComparer.Create(
                    new CultureInfo(string.IsNullOrEmpty(orderLanguageCode)
                        ? DomainConstants.DefaultLanguage
        ...
        public IEnumerable<OntologyTerm> GetById(IEnumerable<Guid> ids)
        {
            GetData();
            foreach (var id in ids)
            {
                yield return Data.TryGetOrDefault(id);
            }
        }
