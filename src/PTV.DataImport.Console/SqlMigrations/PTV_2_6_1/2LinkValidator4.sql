-- Views
CREATE OR REPLACE VIEW "geoServiceChannelWebPage"
AS
SELECT scwp."ServiceChannelVersionedId",
       wpt."Code" AS "WebPageType",
       scwp."Name",
       wp."Url",
       scwp."Description",
...
FROM "ElectronicChannelUrl" AS e
         JOIN "WebPage" AS wp ON e."WebPageId" = wp."Id"
         JOIN "ServiceChannelVersioned" AS ver ON e."ElectronicChannelId" = ver."Id"
         JOIN "PublishingStatusType" AS pst ON ver."PublishingStatusId" = pst."Id"
WHERE wp."IsBroken" = TRUE
  AND (pst."Code" = 'Published' OR pst."Code" = 'Draft' OR pst."Code" = 'Modified')
UNION
...
FROM "PrintableFormChannelUrl" AS e
         JOIN "WebPage" AS wp ON e."WebPageId" = wp."Id"
         JOIN "ServiceChannelVersioned" AS ver ON e."PrintableFormChannelId" = ver."Id"
         JOIN "PublishingStatusType" AS pst ON ver."PublishingStatusId" = pst."Id"
WHERE wp."IsBroken" = TRUE
  AND (pst."Code" = 'Published' OR pst."Code" = 'Draft' OR pst."Code" = 'Modified')
UNION
...
FROM "WebpageChannelUrl" AS e
         JOIN "WebPage" AS wp ON e."WebPageId" = wp."Id"
         JOIN "ServiceChannelVersioned" AS ver ON e."WebpageChannelId" = ver."Id"
         JOIN "PublishingStatusType" AS pst ON ver."PublishingStatusId" = pst."Id"
WHERE wp."IsBroken" = TRUE
  AND (pst."Code" = 'Published' OR pst."Code" = 'Draft' OR pst."Code" = 'Modified');
