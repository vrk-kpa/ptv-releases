...
namespace PTV.DataImport.WinExcelToJson.Tasks
{
    ...
    {
        private const string GeneralDescriptionsStartDataFile = "SourceFiles\\SOTE-generalDescriptions-april-2017.xlsx";
        private readonly string GeneralDescriptionsGeneratedFile = "GeneralDescription.json";

        private List<LanguageLabel> dataSheetNames = new List<LanguageLabel>
        {
            new LanguageLabel {Label = "Fi", Lang = "fi"},
            //new LanguageLabel {Label = "SV", Lang = "sv"},
            //new LanguageLabel {Label = "EN", Lang = "en"},
        };
...
//                    foreach (ExcelWorksheet worksheet in package.Workbook.Worksheets)
                {
                    ...
                    var worksheet =
                        package.Workbook.Worksheets.FirstOrDefault(x => x.Name.ToLower() == dataSheet.Label.ToLower());
                            ...
                            do
                            {
                                ...
                                if (defaultData)
                                {
                                    ...
                                    ProcessTerms(
                                        ...
                                        x =>
                                            new KeyValuePair<string, string>(x.Key.Replace(".html", string.Empty),
                                                x.Value)
...
//                            return dataItem;
        ...
        private void ProcessTerms(string data, List<KeyValuePair<string, string>> terms,
            Func<KeyValuePair<string, string>, KeyValuePair<string, string>> updateFunc = null)
        {
        ...
        private void UpdateLaws(ExcelWorksheet reader, List<Law> lawsList, string language, int row)
        {
            ...
            if (lawLinks.Count != lawsList.Count)
            {
                ...
                Console.Error.WriteLine(
                    $"Wrong list of laws for languages. {language} contains {lawLinks.Count} instead of {lawsList.Count}.");
                return;
            }
